...
----
## Introduction
This project was made as part of EPFL's Large-scale data science for real-world data course (COM-490) by 6 students: 
...
Then you need to run [User_Interface](notebooks/5_user-interface/User_Interface.ipynb), choose the parameters and click on the button to start computing the shortest path.

**Important!**  
As we were not able to produce the network for the full 15km radius around Zürich HB, the run will be done for a radius of 5 km by default.  
More precisely:  
   - To test on a smaller subset, we produced the network for a 1km radius, for which the data is located in the file small_data. To use it, you will need to change the loading path (2 small changes) in [`helpers_UI.py`](notebooks/5_user-interface/helpers_UI.py) and [`helpers.py`](notebooks/5_user-interface/helpers.py).
   - To test on a larger subset,  we produced the network for a 5km radius, for which the data is located in the file 5km_data. To use it, you will need to change the loading path (2 small changes) in [`helpers_UI.py`](notebooks/5_user-interface/helpers_UI.py) and [`helpers.py`](notebooks/5_user-interface/helpers.py).

If you want to run for 15 km, you will need to run the whole project (all files).
To run this project, you will need to:
1. Run first the python script [`1_setup-database.md`](notebooks/1_setup-database.md) to setup the database. 
2. Then the python script [`2_make-tables.py`](notebooks/2_make-tables.py) to preprocess the data and construct tables that will be used later on. 
3. Then the python script [`3_model-delays-prediction.py`](notebooks/3_model-delays-prediction.py) which create the predictive models. 
4. Then the python script [`4_shortest-paths.ipynb`](notebooks/4_shortest-paths.ipynb) which construct the transport network. It also contains testings on the route planning algorithm and predictive model.

Link to the video presentation : https://moodle.epfl.ch/mod/assign/view.php?id=1152777
